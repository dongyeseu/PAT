...
int main()
{
    ...
    for(i=0 ; i<N ; i++)
    {
        ...
        for(j=0 ; j<6 ; j++)
        {
            getline(cin,Info[j]);
            if(j>0 && j!=3)
                Book[j-1][Info[j]].push_back(Info[0]);
            else if(j == 3)
            {
                string a = "";
                for(char c : Info[j])
    ...
    for(i=0 ; i<K ; i++)
    {
        ...
        int num = query[0]-'0';
        string qry = query.substr(3,query.size()-3);
        vector<string>Res(Book[num-1][qry].begin(),Book[num-1][qry].end());
        cout<<query<<"\n";
        if(Res.empty())
            cout<<"Not Found\n";
        else
