...
int main()
{
    vector<vector<struct People>>Rank(4);
    int N,L,H;
    cin>>N>>L>>H;
    int i;
    int sum = 0;
    ...
    {
        struct People temp;
        scanf("%d %d %d",&temp.Name,&temp.virtue,&temp.talent);
        if(temp.virtue >= L && temp.talent >=L)
        {
            if(temp.virtue>=H && temp.talent >= H)
            {
                Rank[0].push_back(temp);
            }
            else if(temp.virtue >= H && temp.talent < H)
            {
                Rank[1].push_back(temp);
            }
            else if(temp.virtue<H && temp.talent < H && temp.talent <= temp.virtue)
            {
                Rank[2].push_back(temp);
            }
            else
            {
                Rank[3].push_back(temp);
            }
            sum++;
        }
    }
    ...
    for(i=0 ; i<4; i++)
    {
        ...
        int Size = Rank[i].size();
        int j;
        for(j=0 ; j<Size ; j++)
            printf("%d %d %d\n",Rank[i][j].Name,Rank[i][j].virtue,Rank[i][j].talent);
    }
    return 0;
}
