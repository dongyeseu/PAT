/*
1004 Counting Leaves (30)£¨30 ·Ö£©ÌáÎÊ
...
int main()
{
    int N,M,i,j,root;
    cin>>N>>M;
    vector<struct Family>Tree(N+1);     //数组型二叉树
    vector<int>Book(N+1,0); //确认根节点
    int temp,K,cd;
    for(i=0 ; i<M ; i++)
    {
        cin>>temp;
        cin>>K;
        Tree[temp].Child = (int *)malloc(sizeof(int)*K);
        Tree[temp].Leaf = true;
        Tree[temp].num = K;
        for(j=0 ; j<K ; j++)
        {
