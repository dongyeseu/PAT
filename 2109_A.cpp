...
int main()
{
    ...
    cin>>N>>K;
    int i;
    vector<struct student>Class;
    for(i=0 ; i<N ; i++)
    {
        struct student tmep;
        cin>>tmep.name>>tmep.height;
        Class.push_back(tmep);
    }
    ...
    for(i=0 ; i<K ; i++)
    {
        ...
        if(i!=0)
            ...
            sums = rows + complement;
        string nums[sums];
        int k,z,flag=1;
        for(k=sums/2,z=0 ; k<sums&&k>=0 ; k = k+flag*z)
        {
            nums[k] = Class[j++].name;
            flag = -flag;
            z++;
